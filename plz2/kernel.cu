...
cudaSetDevice(0);	
cudaDeviceProp prop;
cudaGetDeviceProperties(&prop, 0); // 0번 디바이스 사용.
printf("max num thread per block %d max size dim of a block %d dim[1] %d dim[2] %d dim[3] %d \n",
    prop.maxThreadsPerBlock,prop.maxGridSize,prop.maxThreadsDim[0],prop.maxThreadsDim[1],prop.maxThreadsDim[2]);
...
int thrdperblock = block_number;

printf("\nbefore div 4 thrdperblock %d \n",block_number);
printf("\nafter div 4 thrdperblock %d \n",thrdperblock);
...
cudaEvent_t startAllocate, endAllocate;

float allocate_time;

cudaEventCreate(&startAllocate);
cudaEventCreate(&endAllocate);

cudaEventRecord(startAllocate,0);
...
cudaEventRecord(endAllocate,0);
cudaEventSynchronize(endAllocate);
cudaEventElapsedTime(&allocate_time, startAllocate, endAllocate);

printf("\nallocate time %f ms \n",allocate_time);

//printf("BlockperGrid %d, ThreadperBlock %d \n",BlockperGrid,ThreadperBlock);
//int nTestThreads = (int)(prop.maxThreadsPerBlock / 4);
//printf("BlockperGrid 655335, nTestThreads %d \n",nTestThreads);
...
AES_Encrypt <<< testblock, ThreadperBlock>>>(cuda_aes_block_array, cuda_key, expandKeyLen, block_number);
